...
kubectl apply -f "C:\github\Network-policies-for-hybrid-node-pool-cluster\Linux only cluster using azure npm\Case-1-Allow-Ingress-from-some-pods-in-same-namespace\Serverpods.yaml"
kubectl apply -f "C:\github\Network-policies-for-hybrid-node-pool-cluster\Linux only cluster using azure npm\Case-1-Allow-Ingress-from-some-pods-in-same-namespace\Testpods.yaml"
kubectl get pods -n development --show-labels 

# Apply NP
kubectl apply -f "C:\github\Network-policies-for-hybrid-node-pool-cluster\Linux only cluster using azure npm\Case-1-Allow-Ingress-from-some-pods-in-same-namespace\Networkpolicy.yaml"
